        ...
        <div class="row">
            ...
            <div class="col-md-9">
                <ul class="nav nav-cabinet fs-15">
                    <li class="nav-item position-relative active  pb-30 mr-15">
                        <a class="nav-link <?= $status != "finished" ? 'active' : '' ?> p-0" href="<?= toRoute('/cabinet/order') ?>"><?= t("Активные заявки") ?> (<?= $active_count ?>)</a>
                    </li>
                    <li class="nav-item position-relative pb-30 mx-15">
                        <a class="nav-link p-0 <?= $status == "finished" ? 'active' : '' ?>" href="<?= toRoute('/cabinet/order?status=finished') ?>"><?= t("Завершенные заявки") ?> (<?= $finished_count ?>)</a>
                    </li>
                </ul>
                ...
                <div class="i-list mb-30 mt-10">
                    <div class="i-list__head px-20 mb-20">
                        <div class="row no-gutters">
                            <div class="col-2">
                                <div class="i-list__sortable"><?= t("Номер и дата заказа") ?></div>
                            </div>
                            ...
                            <div class="col-2 text-right">
                                <div class="i-list__sortable"><?= t("Общее сумма") ?></div>
                            </div>
                                ...
                                <div class="item shadow bg-white rounded mb-10 wow fadeInUp">
                                    <div class="item__head py-15 px-20 ">
                                        <div class="row no-gutters align-items-center">
                                            <div class="col-2">
                                                <a href="<?= toRoute('/cabinet/order/' . $order->id) ?>" class="item__name"><b>№ <?= $order->id ?></b> / <span><?= date("d.m.Y H:i", strtotime($order->created_at)) ?></span></a>
                                            </div>
                                            <div class="col text-center">
                                                <b><?= $order->receiver_fio ?></b>
                                            </div>
                                            <div class="col text-center"><?= showNorm($order->address ? $order->address->text : $order->address_text) ?></div>
                                            <div class="col-2 text-center">
                                                <span class="item__badge <?= $order->statusClass ?> item__badge--status-3 d-inline-block"><?= $order->statusName ?></span>
                                            </div>
                                            <div class="col-2 text-right">
                                                <div class="item__price"><?= showPrice($order->total_sum) ?> <small><?= t("сум") ?></small></div>
                                            </div>
                                            <div class="col-auto pl-15">
                                                <div class="item__icon collapsed" data-toggle="collapse" data-target="#itemId-<?= $order->id ?>" role="button" aria-expanded="false" aria-controls="itemId-<?= $order->id ?>">
                                                ...
                                                <div class="row align-items-center">
                                                    <div class="col text-right">
                                                        <div class="collapse-btns">
                                                            ...
                                                            <a href="<?= toRoute('/cabinet/order/' . $order->id) ?>" class="btn btn-primary px-20 py-5 fs-15 mr-5"><?= t("Посмотреть") ?></a>
                                                            <a class="btn btn-outline-secondary rounded-pill px-20 py-5 fs-15 mr-5" href="<?= toRoute('/cabinet/order/update/' . $order->id) ?>">
                                                                <?= t('Редактировать') ?>
                                                            </a>
                                                            <a class="btn btn-outline-danger rounded-pill px-20 py-5 fs-15 mr-5" data-method="POST" data-confirm="<?= t("Вы уверены?") ?>" href="<?= toRoute('/cabinet/order/delete/' . $order->id) ?>">
                                                                <?= t('Удалить') ?>
                                                            </a>
                                                        </div>
                                                    </div>
