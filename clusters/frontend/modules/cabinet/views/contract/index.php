            ...
            <div class="col-md-9">
                <ul class="nav nav-cabinet fs-15">
                    <!-- <li class="nav-item  position-relative  pb-30 mr-15">
                        <a class="nav-link active p-0" href="<?= toRoute('/' . Yii::$app->controller->module->id . '/contract') ?>">
                                ...
                                <div class="item shadow bg-white rounded mb-10 wow fadeInUp">
                                    <div class="item__head py-15 px-20 ">
                                        <div class="row no-gutters align-items-center">
                                            ...
                                            <div class="col-2 text-center"><?= $model->order->deliveryType ?></div>
                                            <div class="col-2 text-center">
                                                <span class="item__badge <?= $model->statusClass ?> d-inline-block"><?= $model->statusName ?></span>
                                                <span class="mt-5 d-block fs-14">
                                                    <?php if ($model->customer_signed) : ?>
                                                        <?= t("Ожидание поставщика") ?>
                                                    <?php elseif ($model->producer_signed) : ?>
                                                        <?= t("Ожидание заказчика") ?>
                                                    <?php else : ?>
                                                        <?= t("Ожидание обе стороны") ?>
                                                    <?php endif ?>
                                                </span>
                                            </div>
                                                ...
                                                <div class="row align-items-center">
                                                    <div class="col text-right">
                                                        <div class="collapse-btns">
                                                            ...
                                                            <?php if (!$model->producer_signed) : ?>
                                                                <a class="btn btn-secondary px-20 py-5 fs-15" data-confirm="<?= t("Вы точно хотите подписать?") ?>" data-sign="true" data-tin="<?= $model->producer->tin ?>" href="<?= toRoute('/' . Yii::$app->controller->module->id . '/contract/accept?id=' . $model->id) ?>"><?= t("Подписать") ?></a>
                                                            <?php else : ?>
                                                                <span>
                                                                    <?= t("Вы подписали") ?>
                                                                </span>
                                                            <?php endif ?>
                                                            <a class="btn btn-danger px-20 py-5 fs-15" data-sign="true" data-tin="<?= $model->producer->tin ?>" data-confirm="<?= t("Вы точно хотите расторгнуть?") ?>" href="<?= toRoute('/' . Yii::$app->controller->module->id . '/contract/cancel?id=' . $model->id) ?>">
                                                                <?= t("Расторгнуть") ?>
                                                            </a>
                                                        </div>
                                                    </div>
