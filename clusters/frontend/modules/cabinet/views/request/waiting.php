            ...
            <div class="col-md-9">
                <ul class="nav nav-cabinet fs-15">
                    <li class="nav-item position-relative  pb-30 mr-15">
                        <a class="nav-link active p-0" href="<?= toRoute('/cabinet/request/waiting') ?>">
                ...
                <div class="i-list mb-30 mt-10">
                    <div class="i-list__head px-20 mb-20">
                        <div class="row no-gutters">
                            ...
                            <div class="col-2 text-center">
                                <div class="i-list__sortable"><?= t("Дата окончания удтверждения") ?></div>
                            </div>
                                ...
                                <div class="item shadow bg-white rounded mb-10 wow fadeInUp">
                                    <div class="item__head py-15 px-20 ">
                                        <div class="row no-gutters align-items-center">
                                            <div class="col-2">
                                                <a href="<?= toRoute('/cabinet/request/' . $order->id) ?>" class="item__name"><b>№ <?= $order->id ?></b> / <span><?= date("d.m.Y H:i", strtotime($order->created_at)) ?></span></a>
                                            </div>
                                            <div class="col-3 text-center">
                                                <?php foreach ($order->orderLists as $index => $order_list) : ?>
                                                    <a href="<?= toRoute('/cabinet/request/' . $order->id) ?>"><?= $order_list->product->title ?></a>
                                                <?php endforeach ?>
                                            </div>
                                            ...
                                            <div class="col-2 text-center">
                                                <span class="text-danger">
                                                    <?= $order->request_end ? date("d.m.Y H:i", strtotime($order->request_end)) : null ?>
                                                </span>
                                            </div>
                                                ...
                                                <div class="row align-items-center">
                                                    <div class="col text-right">
                                                        <div class="collapse-btns">
                                                            ...
                                                            <a href="<?= toRoute('/cabinet/request/' . $order->id) ?>" class="btn btn-primary px-20 py-5 fs-15 mr-5"><?= t("Посмотреть") ?></a>

                                                            <a class="btn btn-secondary px-20 py-5 fs-15 mr-5" data-sign="true" data-tin="<?= $order->company->tin ?>" data-confirm="<?= t("Вы точно хотите принять? Начнётся 48-часовой торг. Мы снимем с вашего баланса {currency} сум, {deposit_percentage}% в виде залога и {commission_percentage}% в виде комиссии от стартовой суммы.", [
                                                                                                                                                                                            'currency' => showPrice($order->total_sum * ($deposit_percentage + $commission_percentage) / 100),
                                                                                                                                                                                            'deposit_percentage' => $deposit_percentage,
                                                                                                                                                                                            'commission_percentage' => $commission_percentage
                                                                                                                                                                                        ]) ?>" href="<?= toRoute('/cabinet/request/accept?id=' . $order->id) ?>">
                                                                <?= t("Принять") ?>
                                                            </a>
                                                            <a class="btn btn-outline-danger rounded-pill px-20 py-5 fs-15 mr-5" data-sign="true" data-tin="<?= $order->company->tin ?>" data-confirm="<?= t("Вы точно хотите отказать?") ?>" href="<?= toRoute('/cabinet/request/cancel?id=' . $order->id) ?>">
                                                                <?= t("Отказать") ?>
                                                            </a>
                                                        </div>
                                                    </div>
