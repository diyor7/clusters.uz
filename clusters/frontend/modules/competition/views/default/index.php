...
<div class="new-bg-different">
    ...
    <div class="container  pt-40 mh-40">
                ...
                <form action="" class="w-100">
                    <div class="d-flex flex-wrap">
                        ...
                        <div class="form-group mb-15 col-2 mr-25 px-0">
                            <label for="a-name5"><?=t("ТН ВЭД")?>:</label>
                            <select name="tn_id" class="tn-ajax w-100" value="<?= $tn_id ?>">
                                <?php $tn = \common\models\Tn::findOne($tn_id); ?>

                                <?php if ($tn) : ?>
                                    <option value="<?= $tn->id ?>"><?= $tn->code . ' - ' . $tn->title ?></option>
                                <?php endif ?>
                            </select>
                        </div>
        ...
        <table class="new-table mt-35 bg-white">
            <thead>
                <tr>
                    <th>№</th>
                    <th><?= t("Номер лота") ?></th>
            ...
            <tbody>
                ...
                <?php foreach ($models as $index => $model) : ?>
                    <tr class="bg-white" onclick="document.location.href = '<?= toRoute('/competition/view/' . $model->id) ?>'">
                        <td>
                            <?= $index + 1 ?>
                        </td>
                        <td>
                        ...
                        <td>
                            <?php foreach ($model->auctionTns as $index => $model_list) : ?>
                                <span class="mr-2"><?= $model_list->tn->title ?><?= $index != count($model->auctionTns) - 1 ? ', ' : '' ?></span>
                            <?php endforeach ?>
                        </td>
                        <td>
