...
<div class="new-bg-different">
    <div class="container py-30">
        ...
        <div class="new-breadcrumbs d-flex align-items-center">
            ...
            <span></span>
            <?php if ($type==0):?>
                <a href="<?=Url::to(['/organisation/ngmk/index', 'id' => $company->id, 'type' => 0])?>"><?php echo Yii::t('frontend', "Импорт");?></a>
            <?php elseif($type==2):?>
                <a href="<?=Url::to(['/organisation/ngmk/index', 'id' => $company->id, 'type' => 2])?>"><?php echo Yii::t('frontend', "Местные закупки");?></a>
            <?php elseif($type==1):?>
                <a href="<?=Url::to(['/organisation/ngmk/index', 'id' => $company->id, 'type' => 1])?>"><?php echo Yii::t('frontend', "Выпускаймая продукция");?></a>
            <?php else:?>
                <a href="#"><?=$this->title?></a>
            <?php endif;?>
        </div>
...
<!--        <h1 class="page-title font-weight-bold fs-36 pb-25" id="page-name"></h1>-->
            <ul class="nav mb-45 category__types">
                <li class="nav-item <?=$type==0?'active':''?>">
                    <a href="<?=Url::to(['/organisation/ngmk/index', 'id' => $company->id, 'type' => 0])?>" class="nav-link px-0 mr-15 fs-24 fw-semi-bold"><?php echo Yii::t('frontend', "Импорт");?></a>
                </li>
                <li class="nav-item <?=$type==2?'active':''?>">
                    <a href="<?=Url::to(['/organisation/ngmk/index', 'id' => $company->id, 'type' => 2])?>" class="nav-link px-0 mr-15 fw-semi-bold fs-24"><?php echo Yii::t('frontend', "Местные закупки");?></a>
                </li>
                <li class="nav-item <?=$type==1?'active':''?>">
                    <a href="<?=Url::to(['/organisation/ngmk/index', 'id' => $company->id, 'type' => 1])?>" class="nav-link px-0 fs-24 fw-semi-bold"><?php echo Yii::t('frontend', "Выпускаймая продукция");?></a>
                </li>
            </ul>
                            ...
                            <div class="item rounded-lg position-relative d-flex align-items-center p-20 wow fadeInUp">
                                ...
                                <div class="item__content">
                                    <div class="item__name">
                                        <a href="<?=Url::to(['/organisation/ngmk/products', 'id' => $company->id, 'category_id' => $category->id, 'type' => $type])?>" class="item__link stretched-link"><?=$category->name?></a>
                                    </div>
