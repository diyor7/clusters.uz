...
<div class="container z-index-1 bg-white position-relative px-40 mb-35 py-35 mt-150 overflow-hidden">

    <section class="pt-0 mb-60 pb-0 category">
        <h1 class="page-title font-weight-bold fs-36 pb-25" id="page-name"><?=$type==1?Yii::t('frontend', 'Выпускаемая продукция'):Yii::t('frontend', 'Закупки')?></h1>
        <?php if ($type!=1):?>

            <ul class="nav mb-45 category__types">
                <li class="nav-item pl-0 <?=$type==0?'active':''?>">
                    <a href="<?=Url::to(['/site/organisation-buys', 'id' => $company->id, 'type' => 0])?>" class="nav-link fs-24 fw-semi-bold"><?php echo Yii::t('frontend', "Импорт");?></a>
                </li>
                <li class="nav-item <?=$type==2?'active':''?>">
                    <a href="<?=Url::to(['/site/organisation-buys', 'id' => $company->id, 'type' => 2])?>" class="nav-link fw-semi-bold fs-24"><?php echo Yii::t('frontend', "Местные закупки");?></a>
                </li>
            </ul>
            <div>
                <?php if ($type==1):?>
                            ...
                            <div class="item rounded-lg position-relative d-flex align-items-center p-20 wow fadeInUp">
                                ...
                                <div class="item__img rounded-circle shadow mr-20">
                                    <?php if(!empty($category->image)):?>
                                        <div class="item__img-holder" style="background-image: url('/uploads/images/category/<?=!empty($category->imagecol)?$category->imagecol:$category->image?>')"></div>
                                ...
                                <div class="item__content">
                                    <div class="item__name">
                                        <a href="<?=Url::to(['/site/organisation-products', 'id' => $company->id, 'category_id' => $category->id, 'type' => $type])?>" class="item__link stretched-link"><?=$category->name?></a>
                                    </div>
